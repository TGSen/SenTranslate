...
public class ClipImageView extends AppCompatImageView implements
    ...
    private ClipMatrix mScaleMatrix = new ClipMatrix();
    ...
    private class AutoScaleRunnable implements Runnable {
        ...
        public AutoScaleRunnable(float targetScale, float x, float y) {
            ...
            if (getScale() < mTargetScale) {
                Log.e("Harrison", "BIGGER" + getScale() + " targetScale:" + targetScale);
                tmpScale = BIGGER;
            } else {
                Log.e("Harrison", "SMALLER" + getScale() + " targetScale:" + targetScale);
                tmpScale = SMALLER;
            }
    ...
    private RectF getMatrixRectF() {
        ClipMatrix matrix = mScaleMatrix;
    ...
    public ClipMatrix getScaleMatrix() {
        return mScaleMatrix;
    }
    ...
    public final float[] getCurrentState() {
        float[] floats = new float[3];
        mScaleMatrix.getValues(mMatrixValues);
        floats[0] = mMatrixValues[Matrix.MTRANS_X];
        floats[1] = mMatrixValues[Matrix.MTRANS_Y];
        floats[2] = mMatrixValues[Matrix.MSCALE_X];
        return floats;
    }
    ...
    public void setImageUriByMatrix(Uri uri,ClipMatrix matrix){
        super.setImageURI(uri);
        setImageMatrix(matrix);
        this.mScaleMatrix = matrix;
        postResetTargetImageMatrix();
    }

    /**
     *
     */
    private void postResetTargetImageMatrix() {
        if (getWidth() != 0) {
            resetImageTargetMatrix();
        } else {
            post(new Runnable() {
                @Override
                public void run() {
     ...
     * 垂直方向与View的边矩
     */
    public void resetImageTargetMatrix() {
    ...
    public Bitmap clip() {
        ...
        Log.e("Harrsion","********************"+cropX+"****"+(int) cropY+"****"+(int) cropWidth+"****"+(int) cropHeight);
